...
import { SimilarAnalysesPanel } from "./components/SimilarAnalysesPanel";
...
export default function App() {
  ...
  // Moorcheh community memory state
  const [chatOpen, setChatOpen] = useState(false);
  const [showNeighborhoodSummary, setShowNeighborhoodSummary] = useState(false);
  const [lastAnalyzedBuilding, setLastAnalyzedBuilding] = useState<{
    location: [number, number];
    type: string;
    height: number;
    footprint: number;
  } | null>(null);
  const [showSimilarAnalyses, setShowSimilarAnalyses] = useState(false);

  const storeAnalysisInMemory = useCallback(async (analysisData: {
    location: [number, number];
    buildingType: string;
    height: number;
    footprint: number;
    stories: number;
    trafficImpact: Record<string, unknown>;
    ...
    try {
      await fetch("http://localhost:3001/api/moorcheh/store-analysis", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          location: analysisData.location,
          building_type: analysisData.buildingType,
          height: analysisData.height,
          footprint: analysisData.footprint,
          stories: analysisData.stories,
          traffic_impact: analysisData.trafficImpact,
          air_quality: analysisData.airQuality,
          noise: analysisData.noise,
      ...
      console.log("[Moorcheh] Analysis stored in community memory");
      setLastAnalyzedBuilding({
        location: analysisData.location,
        type: analysisData.buildingType,
        height: analysisData.height,
        footprint: analysisData.footprint,
      });
      setShowSimilarAnalyses(true);
    } catch (err) {
      console.warn("[Moorcheh] Failed to store analysis (service may be offline):", err);
    }
  }, []);
  ...
  return (
    <div className="app-shell">
      ...
      <SimulationResultsPanel
        ...
        onAnalysisComplete={(data) => {
          storeAnalysisInMemory({
            ...
            height: 30,
            footprint: 400,
            stories: Math.round(30 / 3.5),
            trafficImpact: { closedRoads: data.closedRoads, buildingCount: data.buildingCount },
            airQuality: {},
            noise: {},
            economicImpact: {},
            overallRisk: data.closedRoads > 3 ? "high" : data.closedRoads > 1 ? "medium" : "low",
            severity: Math.min(10, 3 + data.closedRoads * 2),
      ...
      <button
        ...
        title="Open Community Memory Chat"
        style={{
          position: "fixed",
          bottom: 20,
          right: 20,
          ...
          height: 52,
          borderRadius: "50%",
          background: chatOpen ? "#dc3545" : "#4a6cf7",
          color: "#fff",
          border: "none",
          cursor: "pointer",
          fontSize: 24,
      ...
      {/* Similar Analyses Panel */}
      <SimilarAnalysesPanel
        buildingLocation={lastAnalyzedBuilding?.location ?? null}
        buildingType={lastAnalyzedBuilding?.type ?? ""}
        buildingHeight={lastAnalyzedBuilding?.height ?? 0}
        buildingFootprint={lastAnalyzedBuilding?.footprint ?? 0}
        isVisible={showSimilarAnalyses}
      />

      {/* Neighborhood Summary Overlay */}
      <NeighborhoodSummary
        isVisible={showNeighborhoodSummary}
        centerPoint={(() => {
          if (polygonBuildings.size === 0) return undefined;
          const buildings = Array.from(polygonBuildings.values());
          let tLng = 0, tLat = 0, cnt = 0;
          buildings.forEach((f) => {
            if (f.geometry.type === "Polygon") {
              const coords = f.geometry.coordinates[0];
              coords.forEach((c: number[]) => { tLng += c[0]; tLat += c[1]; cnt++; });
            }
          });
          return cnt > 0 ? [tLng / cnt, tLat / cnt] as [number, number] : undefined;
        })()}
        onToggle={() => setShowNeighborhoodSummary((prev) => !prev)}
      />
    </div>
  );
}
